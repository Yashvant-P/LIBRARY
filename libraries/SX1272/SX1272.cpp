...
uint8_t SX1272::getACK(uint16_t wait)
{
    ...
    if( a_received )
    {
		...
		ACK.data[2] = readRegister(REG_FIFO);
		ACK.data[3] = readRegister(REG_FIFO);
		ACK.data[4] = readRegister(REG_FIFO);
		ACK.data[5] = readRegister(REG_FIFO);
                    ...
                    if( ACK.packnum == packet_sent.packnum )
                    {
                        if( ACK.length == 2 )
                        {
                            if( ACK.data[0] == CORRECT_PACKET )
                            {
								...
								Serial.println("\n\n\n\n\\n\n\n\n\n\\n ACK data");
								Serial.println(ACK.data[2]);
								Serial.println(ACK.data[3]);
								Serial.println(ACK.data[4]);
								Serial.println(ACK.data[5]);
                                value = ACK.data[1];
